...
async function add_element_in_sorted_object_array_by_property(data, method = 'add element in sorted object array by property') {
    let output;
    try {
        new validator(SortLib.add_element_in_sorted_object_array_by_property(data.scope, data.property, data.argument).array)
            .is_same(data.target)
            .and().bind(
                new validator(SortLib.add_element_in_sorted_object_array_by_property(inverse_array(data.scope), data.property, data.argument).array)
